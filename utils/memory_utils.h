...
static void truncate_file_to_size(int fd, unsigned long size) {
...
static void prealloc_file(int fd, unsigned long size) {
  if (size == 0) {
    return;
  }
  int ret = posix_fallocate(fd, 0, size);
  if (ret != 0) {
    BOOST_LOG_TRIVIAL(fatal) << " posix_fallocate failed:" <<
    strerror(ret);
    exit(-1);
  }
}

static void unlink_file(char *fname) {
