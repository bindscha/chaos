...
namespace algorithm {
  ...
  void scatter_gather<A, F>::operator()() {
    ...
    if (vm["benchmark"].as<std::string>() == "bfs") {
      ...
      strcpy(format, "LLLL");
      unsigned long niters = 1 + vm["pagerank::niters"].as < unsigned
      long > ();
      PyObject_CallMethod(pInstance, funName, format, x_lib::configuration::partition_shift,
  ...
  void scatter_gather<A, F>::partition_callback
    ...
    switch (sg_pcpu::current_step) {
      ...
      case phase_scatter: {
        ...
        pReturnValue = PyObject_CallMethod(pInstance, functionNamePy, format, pMemVertices, pMemInput, pMemOutput,
                                           pParamBuffer);
